...
V38: blog `title` (frontmatter @ `src/content/blog/<slug>.md`) ∧ mirrored linkedin `Blog:` line per V36 ∧ `description` frontmatter ! self-disambiguate — bare domain noun w/ ≥ 2 plausible dimension readings (e.g. `compression`, `drift`, `decoder`) ! qualifier specifying dimension; reader landing cold via search ∨ social-card share ! grasp subject pre-body; slug ⊥ rename to fix (V23 URL-break rule).
...
T18|.|retitle measuring-glyph-compression post per V38 — scope: `rg 'Measuring math-glyph compression(?! token)' src/content/blog/*.md linkedin/*.txt`; remediation ≡ insert `token` qualifier → `Measuring math-glyph token compression` (frontmatter title + V36-mirrored Blog: line) ∧ amend frontmatter `description` to carry dimension marker (e.g. `compresses tokens` ∨ `token compression`); slug ⊥ rename per V23|V38,B10
