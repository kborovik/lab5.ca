...
V36: LinkedIn feed post ≡ `linkedin/<slug>.txt` where `<slug>` ≡ matching `src/content/blog/<slug>.md`; format ≡ plain text + line breaks (LinkedIn ⊥ render markdown); body ⊥ `word.tld` pattern — LinkedIn auto-linkifies (`SPEC.md`, `lab5.ca`, `github.com`), quotes/backticks ⊥ escape; rephrase ∨ drop TLD; body ⊥ markdown syntax (`**bold**`, `` `code` ``, `[text](url)`, `# heading`, `- bullet`) → LinkedIn renders literal ∨ strips; voice ≡ §V.3; structure ≡ hook → body → trailing link block w/ `Write-up: https://lab5.ca/blog/<slug>/` (canonical) ∧ optional additional `<Label>: <url>` rows (e.g. `Benchmark:`, `Plugin:`); `#hashtag` ∧ `@mention` ≡ LinkedIn syntax (sparingly). `linkedin/resume.txt` ≡ profile content, ⊥ scope.
...
B3|2026-05-19|linkedin post body referenced `SPEC.md` ∈ `linkedin/{measuring-glyph-compression,spec-driven-development}.txt`; LinkedIn auto-linkified `SPEC.md` (any `word.tld` matches); single-quote `'SPEC.md'` ⊥ defeat matcher (re-linkified on edit); backtick `` `SPEC.md` `` ⊥ defeat either; resolved by drop `.md` → `SPEC`|V36
